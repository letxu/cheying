(function(config) {
    ...
    var La = Ga ? "android" : Ba ? "ios" : oa ? "windows" : va ? "mac" : "other"
      ...
      , Ab = function() {
        ...
        for (var a = ["webgl", "experimental-webgl", "moz-webgl"], b = null, c = 0; c < a.length; c += 1) {
            ...
            if (b) {
                if (b.drawingBufferWidth !== rb.width || b.drawingBufferHeight !== rb.height)
                    break;
      ...
      , Qb = !0;
    try {
        if ("undefined" === typeof e.localStorage)
            Qb = !1;
        else {
    ...
    config.Fd = "main";
    config.j.vd && (Ub += ",touch",
    config.Fd += "t");
    config.j.U || (Ub += ",mouse",
    config.Fd += "m");
    config.Fd += "c";
    config.j.ne && (config.Fd += "v",
    Ub += ",vectorlayer,overlay",
    config.j.Zd ? (config.Fd += "w",
    Ub += ",wgl") : (config.Fd += "cg",
    Ub += ",cmng,cgl"));
    if (config[7]) {
    ...
    if (oc) {
        ...
        document.write("<link rel='stylesheet' href='" + nc + "'/>");
    function qc(a) {
        var b = document
    ...
    function rc() {
        ...
        config[5] || c && c.async ? qc(a) : (document.write('<script id="amap_main_js" src=\'' + a + "' type='text/javascript'>\x3c/script>"),
        setTimeout(function() {
    ...
    try {
        if (window.localStorage) {
            var tc = window.localStorage["_AMap_" + config.Fd]
              , uc = !1;
            tc ? (tc = JSON.parse(tc),
            tc.version === config.wg ? (eval(tc.script),
            Zb.loaded = !0) : uc = !0) : uc = !0;
            if (uc)
                for (Xb in window.localStorage)
                    window.localStorage.hasOwnProperty(Xb) && 0 === Xb.indexOf("_AMap_") && window.localStorage.removeItem(Xb)
        }
    } catch (vc) {}
    Zb.loaded || (rc(),
    delete config.AN);
}
)(["336c57b9e3679e2b1bfcb4429e2235b7", [118.344933, 29.188757, 120.721945, 30.566516, 120.153576, 30.287459], "https://webapi.amap.com", 1, "1.3", null, "330100", "", true, true, true, true, "1547559048-1"])
